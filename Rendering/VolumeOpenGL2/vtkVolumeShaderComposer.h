...
namespace vtkvolume
{
  ...
  std::string ComputeClip(vtkRenderer* vtkNotUsed(ren),
    ...
    return std::string(
      "vec4 pos = in_projectionmatrix * in_modelViewMatrix * \n\
                   in_volumeMatrix * vec4(v_vertexPos.xyz, 1); \n\
      gl_Position = pos;"
    );
    }
  ...
  std::string ComputeTextureCoords(vtkRenderer* vtkNotUsed(ren),
    ...
    return std::string(
      "// Assuming point data only. Also, we offset the texture coordinate \n\
       ...
       vec3 uvx = (v_vertexPos - in_volExtentsMin) / \n\
                  (in_volExtentsMax - in_volExtentsMin); \n\
       vec3 delta = in_textureExtentsMax - in_textureExtentsMin; \n\
       v_textureCoords = (uvx * (delta - vec3(1.0)) + vec3(0.5)) / delta;"
    );
    }
  ...
  std::string BaseGlobalsVert(vtkRenderer* vtkNotUsed(ren),
    ...
    "uniform mat4 in_modelViewMatrix; \n\
     ...
     \n\
     uniform vec3 in_volExtentsMin; \n\
     uniform vec3 in_volExtentsMax; \n\
     \n\
  ...
  std::string BaseGlobalsFrag(vtkRenderer* vtkNotUsed(ren),
    ...
    std::string shaderStr = std::string(
      ...
      uniform vec2 in_windowLeftCorner; \n\
      uniform vec2 in_invOriginalWindowSize; \n\
      uniform vec2 in_invWindowSize; \n\
      uniform vec3 in_textureExtentsMax; \n\
      uniform vec3 in_textureExtentsMin; \n\
      \n\
  ...
  std::string BaseInit(vtkRenderer* vtkNotUsed(ren),
    ...
    return std::string(
      ...
      vec3 geoin_dir = computeRayDirection(); \n\
      \n\
      // Multiply the raymarching direction with the step size to get the \n\
      // sub-step size we need to take at each raymarching step  \n\
      g_dirStep = (in_inverseTextureDatasetMatrix * \n\
                    vec4(geoin_dir, 0.0)).xyz * in_sampleDistance; \n\
      \n\
    ...
    if (vol->GetProperty()->GetShade() &&
      ...
      return std::string(" \n\
        ...
        vec4 computeGradient() \n\
          ...
          g2.xyz = g1 - g2.xyz; \n\
          vec3 in_spacing = vec3(in_cellSpacing[0], \n\
                               in_cellSpacing[1],  \n\
                               in_cellSpacing[2]); \n\
          vec3 aspect; \n\
          float avg_spacing = (in_spacing[0] + \n\
                               in_spacing[1] + \n\
                               in_spacing[2])/3.0; \n\
          // Adjust the aspect \n\
          aspect.x = in_spacing[0] * 2.0 / avg_spacing; \n\
          aspect.y = in_spacing[1] * 2.0 / avg_spacing; \n\
          aspect.z = in_spacing[2] * 2.0 / avg_spacing; \n\
          g2.x /= aspect.x; \n\
          g2.y /= aspect.y; \n\
          g2.z /= aspect.z; \n\
      ...
      if (lightingComplexity == 1)
        ...
        shaderStr = std::string(" \n\
          vec4 computeLighting(vec4 color) \n\
            ...
            vec3 ldir = normalize(lightPosObj.xyz - v_vertexPos); \n\
            vec3 vdir = normalize(g_eyePosObj.xyz - v_vertexPos); \n\
            ...
            if (n_dot_l > 0) \n\
              { \n\
              finalColor += in_diffuse * n_dot_l * color.rgb; \n\
              } \n\
            if (n_dot_h > 0) \n\
              { \n\
              finalColor += in_specular * pow(n_dot_h, in_shininess); \n\
              } \n\
            finalColor = clamp(final_colvtkOpenGLGPUVolumeRayCastMapperor, vec3(0.0), vec3(1.0)); \n\
            if (grad.w >= 0.0)\n\
              {\n\
        ...
        shaderStr = std::string("\n\
          vec4 computeLighting(vec4 color)\n\
            ...
            vec4 grad = computeGradient(); \n\
            vec3 normal = (in_texture_to_eye_it * vec4(grad.xyz, 0.0)).xyz; \n\
    ...
    if (!ren->GetActiveCamera()->GetParallelProjection())
      ...
      return std::string(
        "vec3 computeRayDirection() \n\
           { \n\
           return normalize(v_vertexPos.xyz - g_eyePosObj.xyz); \n\
           }");
      }
    else
      {
  ...
  std::string TerminationInit(vtkRenderer* vtkNotUsed(ren),
    ...
    return std::string(
      ...
      const vec3 l_texMin = vec3(0); \n\
      \n\
      // Maximum texture access coordinate \n\
      const vec3 l_texMax = vec3(1); \n\
      \n\
      ...
      vec2 in_frag_tex_coord = (gl_FragCoord.xy - in_windowLeftCorner) * \n\
                               in_invWindowSize; \n\
      vec4 l_depthValue = texture2D(in_depthSampler, in_frag_tex_coord); \n\
      ...
      in_frag_tex_coord = (gl_FragCoord.xy - in_windowLeftCorner) * \n\
                           in_invOriginalWindowSize; \n\
      \n\
      ...
      vec4 l_terminatePoint; \n\
      l_terminatePoint.x = \n\
        (gl_FragCoord.x - in_windowLeftCorner.x) * 2.0 * \n\
        in_invWindowSize.x - 1.0; \n\
      l_terminatePoint.y = \n\
        (gl_FragCoord.y - in_windowLeftCorner.y) * 2.0 * \n\
        in_invWindowSize.y - 1.0; \n\
      l_terminatePoint.z = (2.0 * l_depthValue.x - (gl_DepthRange.near + \n\
                              gl_DepthRange.far)) / gl_DepthRange.diff; \n\
      l_terminatePoint.w = 1.0; \n\
      \n\
      // From normalized device coordinates to eye coordinates. \n\
      // in_projectionmatrix is inversed because of way VT \n\
      // From eye coordinates to texture coordinates \n\
      l_terminatePoint = in_inverseTextureDatasetMatrix * \n\
                         in_inverseVolumeMatrix * \n\
                         in_inverseModelviewMatrix * \n\
                         in_inverseProjectionMatrix * \n\
                         l_terminatePoint; \n\
      l_terminatePoint /= l_terminatePoint.w; \n\
      \n\
      l_terminatePointMax = \n\
        length(l_terminatePoint.xyz - g_dataPos.xyz) / \n\
        length(g_dirStep); \n\
  ...
  std::string TerminationIncrement(vtkRenderer* vtkNotUsed(ren),
    ...
    {
    return std::string(
      "// The two constants l_texMin and l_texMax have a value of \n\
      ...
      // sign function for the calculation (sign(g_dataPos-l_texMin) and \n\
      // sign (l_texMax-g_dataPos)) will give us vec3(1,1,1) at the \n\
                           ...
                           vtkVolume* vtkNotUsed(vol))
  {
    if (!mapper->GetClippingPlanes())
      ...
      {
      return std::string("\n\
        float clippingPlanesTs[48];\n\
        int clippingPlanesSize = int(in_clippingPlanes[0]);\n\
        \n\
        mat4 worldToTextureMat = in_inverseTextureDatasetMatrix *\n\
                                    in_inverseVolumeMatrix;\n\
        for (int i = 0; i < clippingPlanesSize; i = i + 6)\n\
          ...
          \n\
          origin = worldToTextureMat * origin;\n\
          normal = worldToTextureMat * normal;\n\
          \n\
          if (origin[3] != 0.0)\n\
            {\n\
          ...
          \n\
          clippingPlanesTs[i]     = origin[0];\n\
          clippingPlanesTs[i + 1] = origin[1];\n\
          clippingPlanesTs[i + 2] = origin[2];\n\
          \n\
          clippingPlanesTs[i + 3] = normal[0];\n\
          clippingPlanesTs[i + 4] = normal[1];\n\
          clippingPlanesTs[i + 5] = normal[2];\n\
          }");
      }
  }
    ...
    if (!mapper->GetClippingPlanes())
      ...
      return std::string("\n\
        for (int i = 0; i < (clippingPlanesSize) && !l_skip; i = i + 6)\n\
         {\n\
         if (dot(vec3(g_dataPos - vec3(clippingPlanesTs[i],\n\
                                       clippingPlanesTs[i + 1],\n\
                                       clippingPlanesTs[i + 2])),\n\
             vec3(clippingPlanesTs[i + 3],\n\
                  clippingPlanesTs[i + 4],\n\
                  clippingPlanesTs[i + 5])) < 0)\n\
           {\n\
           l_skip = true;\n\
           break;\n\
           }\n\
